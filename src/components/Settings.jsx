...
export default function Settings() {
  ...
  const update = (e) => {
    ...
    setUserSettings((f) => ({ ...f, [name]: value }));
  }

  const toggleDark = () => {
    if (userSettings.dark) {
      setUserSettings((f) => ({ ...f, dark: false }))
    } else {
      setUserSettings((f) => ({ ...f, dark: true }))
      ...
      {showModal && (
        <div className="modal">
          ...
          <button
            ...
            className="secondary"
            onClick={toggleDark}
            title={userSettings.dark ? "Switch to light mode" : "Switch to dark mode"}
          >
            {userSettings.dark ? moon : sun}
          </button>
          <label>
            Name
            <input
          ...
          <label>
            ...
            <input
              ...
              type={inputType}
              name="secret"
              value={userSettings.secret}
              onChange={update}
            />
            <u onClick={() => inputType === "password" ? setInputType("text") : setInputType("password")}>{inputType === "password" ? "Show secret" : "Hide secret"}</u>
          </label>
        </div>
      )}
    </>
